resource "aws_vpc" "my-vpc-class-project" {
...
resource "aws_internet_gateway" "my-ig" {
  vpc_id = aws_vpc.my-vpc-class-project.id
  tags = {
    Name = "terraform-ig"
  }
}
...
resource "aws_eip_association" "eip_assoc" {
  instance_id   = aws_instance.my-instance-1.id
  allocation_id = aws_eip.terraform-eip.id
}

resource "aws_eip_association" "eip_assoc-2" {
  instance_id   = aws_instance.my-instance-2.id
  allocation_id = aws_eip.terraform-eip.id
}
